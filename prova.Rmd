...
```{r Number of movies with the computed b_u, echo = TRUE}
edx %>%
  group_by(userId) %>%
  summarize(b_u = mean(rating)) %>%
  filter(n()>=100) %>%
  qplot(aes(b_u), geom="histogram", bins = 30, color = "black", ylab = "Number of movies", main = "Number of movies with the computed b_u")
```
